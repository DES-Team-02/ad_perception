...
def roi_hough_transform(image, midpoint):
    box_width = 100
    if midpoint[1]==620:
        box_width = 250
    start_point = (midpoint[0]-box_width, midpoint[1])
    ...
    lines = cv.HoughLinesP(image[max(0,start_point[1]):min(720,end_point[1]), max(0,start_point[0]):min(1280,end_point[0])], rho = rho, theta = theta, threshold = threshold,
...
def sliding_windows(image, warped_image, num_windows, xm=320):
    ...
    for i in range(num_windows):
        ...
        lines = roi_hough_transform(im_h, midpoint)
        if lines is not None:
