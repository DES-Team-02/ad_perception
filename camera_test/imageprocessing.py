...
def region_selection(image):
	...
	bottom_left = [cols * 0.26, rows * 0.95]
	top_left	 = [cols * 0.42, rows * 0.5]
	bottom_right = [cols * 0.83, rows * 0.95]
	top_right = [cols * 0.6, rows * 0.5]
	vertices = np.array([[bottom_left, top_left, top_right, bottom_right]], dtype=np.int32)
	# filling the polygon with white color and generating the final mask
	test = cv2.fillPoly(mask, vertices, ignore_mask_color)
	# performing Bitwise AND on the input image and mask to get only the edges on the road
	masked_image = cv2.bitwise_and(image, mask)
	cv2.imshow('Loaded Image', test)
	return masked_image
...
def runImage():
	...
	col_image = cv2.cvtColor( image, cv2.COLOR_BGR2RGB )
	resized_image = cv2.resize(col_image, (1000, 700))
